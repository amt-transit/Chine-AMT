    ...
    try {
        ...
        items.forEach((it, idx) => {
            if (curGrp !== null && it.grp !== curGrp && !curGrp.startsWith('ZZZ')) {
                ...
                html += `<tr class="group-summary-row"><td colspan="5">TOTAL ${curGrp}</td><td>${formatArgent(grpDu)}</td><td>${formatArgent(grpReste)}</td><td>${formatArgent(grpEntree)}</td><td>${formatArgent(grpSortie)}</td><td style="color:${colorBenef}; font-weight:bold;">${formatArgent(benef)}</td> </tr>`;
            ...
            if (it.isDep) {
                ...
                html += `<tr class="${rowClass}"><td>${dS}</td><td>-</td><td>-</td><td>${it.motif}</td><td>Dépense</td><td>-</td><td>-</td><td>-</td><td class="text-red">${formatArgent(m)}</td><td>${btnSuppr}</td></tr>`;
            } else {
        ...
        if (curGrp && !curGrp.startsWith('ZZZ')) { let benef = grpEntree - grpSortie; let colorBenef = benef >= 0 ? '#76ff03' : '#ff5252'; html += `<tr class="group-summary-row"><td colspan="5">TOTAL ${curGrp}</td><td>${formatArgent(grpDu)}</td><td>${formatArgent(grpReste)}</td><td>${formatArgent(grpEntree)}</td><td>${formatArgent(grpSortie)}</td><td style="color:${colorBenef}; font-weight:bold;">${formatArgent(benef)}</td> </tr>`; }
