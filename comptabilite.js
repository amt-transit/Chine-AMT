    ...
    try {
        ...
        items.forEach((it, idx) => {
            if (curGrp !== null && it.grp !== curGrp && !curGrp.startsWith('ZZZ')) {
                ...
                html += `<tr class="group-summary-row"><td colspan="4">TOTAL ${curGrp}</td><td>${formatArgent(grpDu)}</td><td>${formatArgent(grpReste)}</td><td>${formatArgent(grpEntree)}</td><td>${formatArgent(grpSortie)}</td><td style="color:${colorBenef}; font-weight:bold;">${formatArgent(benef)}</td> </tr>`;
            ...
            if (it.isDep) {
                ...
                html += `<tr class="${rowClass} interactive-table-row" onclick='voirHistoriquePaiementViaData("${encodeURIComponent(JSON.stringify({ id: it.id, nom: it.nom, reference: it.reference, history: it.hist }))}")'><td>${dS}</td><td>${it.reference}${recuIcon}</td><td>${it.description}</td><td>${it.nom} ${it.prenom}</td><td>${formatArgent(du)}</td><td style="color:${r > 0 ? 'red' : 'green'}">${formatArgent(r)}</td><td class="text-green">${formatArgent(paye)}</td><td>-</td><td><i class="fas fa-eye"></i></td></tr>`;
            }
        });
        if (curGrp && !curGrp.startsWith('ZZZ')) { let benef = grpEntree - grpSortie; let colorBenef = benef >= 0 ? '#76ff03' : '#ff5252'; html += `<tr class="group-summary-row"><td colspan="4">TOTAL ${curGrp}</td><td>${formatArgent(grpDu)}</td><td>${formatArgent(grpReste)}</td><td>${formatArgent(grpEntree)}</td><td>${formatArgent(grpSortie)}</td><td style="color:${colorBenef}; font-weight:bold;">${formatArgent(benef)}</td> </tr>`; }
        tbody.innerHTML = html;
        let u = type==='aerien'?'Kg':'CBM'; const footerRow = document.createElement('tr'); footerRow.style.cssText = "background-color:#000; color:cyan; font-weight:bold; font-size:1.1em; text-align:center;"; footerRow.innerHTML = `<td colspan="9">GRAND TOTAL (Tous Groupes) : ${GT_Q} Colis  |  ${GT_V.toFixed(2)} ${u}</td>`; tbody.appendChild(footerRow);
