...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(btnAjouterClient) {
        btnAjouterClient.addEventListener('click', function() {
            ...
            const clientData = {
                nom, prenom: document.getElementById('client-prenom').value,
    ...
    if(btnValiderEnvoiGroupe) {
        btnValiderEnvoiGroupe.addEventListener('click', async function() {
            ...
            try {
                ...
                // CHANGEMENT ICI POUR LES REFERENCES (AIR / MRT)
                const prefix = typeEnvoi.startsWith('aerien') ? 'AIR' : 'MRT';

                for (let i = 0; i < envoiEnCours.length; i++) {
                    const client = envoiEnCours[i];
                    const indexStr = String(i + 1).padStart(3, '0');
                    const refComplete = `${prefix}-${indexStr}-${refGroupe}`;

                    let photosURLs = [];
                    // Simulation upload (remplacer par code upload réel si besoin)
                    
                    await db.collection('expeditions').add({
                        reference: refComplete, refGroupe: refGroupe,
                        date: dateEnvoi, type: typeEnvoi,
                        nom: client.nom, prenom: client.prenom, tel: client.tel,
                        ...
                        status: 'En attente', quantiteRecue: 0, poidsRecu: 0, montantPaye: 0
                    });
                }
...
// =======================================================
// COMPTABILITÉ (AVEC TOTAUX ÉTENDUS)
// =======================================================
...
function selectSuggestion(client) { /* Unused helper */ }
