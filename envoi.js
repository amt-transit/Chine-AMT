...
function importerCSV() {
    const input = document.getElementById('csv-input'); const file = input.files[0]; const typeEnvoi = document.getElementById('type-envoi').value;
    if (!typeEnvoi || !file) { alert("Type d'envoi et Fichier requis."); return; }
    ...
    reader.onload = function(e) {
        ...
        rows.forEach((row, index) => {
            const cleanRow = row.trim(); if (!cleanRow) return; 
            const cols = cleanRow.split(","); 
            if (index === 0 && (cols[0].toLowerCase().includes("date"))) return; 
            if (cols.length < 6) return; 
            
            const clean = (val) => (val || "").replace(/^"|"$/g, "").trim();
            let rawDate = clean(cols[0]); 
            if (rawDate.includes('/')) { const parts = rawDate.split('/'); rawDate = `${parts[2]}-${parts[1]}-${parts[0]}`; }
            ...
            const nouveauClient = { 
                ...
                detailsColis: [{ desc: clean(cols[6]), qte: parseInt(clean(cols[7])) || 1, val: poidVol }], 
                photosFiles: [] 
            };
            let prixUnitaire = 0; 
