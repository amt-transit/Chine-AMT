...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const searchInput = document.getElementById('search-listing');
    if (searchInput) searchInput.addEventListener('input', renderListing);

    const sortInput = document.getElementById('sort-listing');
    if (sortInput) sortInput.addEventListener('change', renderListing);
});
...
async function chargerListing() {
    ...
    try {
        ...
        // On stocke tous les documents validés pour la recherche/tri
        allListingDocs = allDocs.filter(d => {
...
function renderListing() {
    ...
    const sortOption = sortInput ? sortInput.value : 'date-desc';
    ...
    // Filtrage
    let filteredDocs = allListingDocs.filter(d => {
        if (!query) return true;
        const searchString = `${d.reference || ''} ${d.numBL || ''} ${d.nom || ''} ${d.prenom || ''} ${d.status || ''}`.toLowerCase();
    ...
    filteredDocs.sort((a, b) => {
        if (sortOption === 'date-desc') {
            return Math.max(getTime(b.dateModification), getTime(b.creeLe)) - Math.max(getTime(a.dateModification), getTime(a.creeLe));
        } else if (sortOption === 'date-asc') {
            return Math.max(getTime(a.dateModification), getTime(a.creeLe)) - Math.max(getTime(b.dateModification), getTime(b.creeLe));
    ...
    if (!html) html = '<tr><td colspan="7" style="text-align:center;padding:20px;color:#888;">Aucun résultat correspondant.</td></tr>';
    tb.innerHTML = html;
}
